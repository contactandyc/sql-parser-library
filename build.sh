...
case "$COMMAND" in
  ...
  semver-check)
    ...
    OLD_SO="$OLD_DIR/build/libsql_parser_library_shared.so"
    NEW_SO="build/libsql_parser_library_shared.so"
  ...
  coverage)
    ...
    llvm-cov report "$COV_BUILD_DIR/libsql_parser_library_debug.a" \
