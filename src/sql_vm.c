...
static sql_ctx_column_t *vm_catalog_router(sql_ctx_t *ctx, const char *table_name, const char *column_name) {
    ...
    for (size_t i = 0; i < cs->num_datasets; i++) {
        ...
        if (ds->is_virtual) {
            ...
            sql_ctx_column_t *col = vm->resolve_column(vm, ds->alias, column_name);
            if (col) {
                col->table_name = aml_pool_strdup(ctx->pool, ds->alias);
                return col;
            }
...
static void execute_nested_loop(sql_ctx_t *ctx, sql_compiled_query_t *compiled,
                         ...
                         sql_dataset_t **datasets, sql_node_t **local_filters, sql_node_t **join_conds,
    ...
    // FETCH NEXT TABLE BY EXECUTION ORDER
    int t_idx = exec_order[step];

    // --- HASH JOIN PROBE PHASE ---
    if (join_maps && join_maps[t_idx]) {
        sql_node_t *probe_key = sql_eval(ctx, join_probe_exprs[t_idx]);

        if (probe_key && !probe_key->is_null) {
            group_node_t lookup = {0};
            lookup.key = probe_key;
            group_node_t *found = group_map_find(join_maps[t_idx], &lookup);
    ...
    sql_node_t *join_cond = join_conds[t_idx];
...
static sql_dataset_t *internal_execute(sql_vm_t *vm, sql_select_t *ast, const char *forced_alias) {
    ...
    // --- QUERY OPTIMIZER: EXECUTION ORDER & HASH JOIN BUILD PHASE ---
    ...
    while (jp) {
        ...
        sql_node_t *cond = join_conds[r_table];

        if (jp->algorithm == JOIN_ALGO_HASH_JOIN && cond && cond->type == SQL_COMPARISON && strcmp(cond->token, "=") == 0) {
            sql_node_t *left = cond->parameters[0];
            sql_node_t *right = cond->parameters[1];

            int left_dep = -2, right_dep = -2;
            check_node_dep(left, &left_dep);
            check_node_dep(right, &right_dep);

            int l_table = (left_dep == r_table) ? right_dep : left_dep;

            if (l_table >= 0) {
                int build_table = r_table;
                sql_node_t *build_expr = (r_table == left_dep) ? left : right;
                sql_node_t *probe_expr = (r_table == left_dep) ? right : left;

                // Find indices in execution sequence
                int idx_L = -1, idx_R = -1;
                ...
                // ONLY Swap if they are adjacent in execution (Safe Heuristic)
                if (idx_L == idx_R - 1) {
                    ...
                    if (score_L < score_R) { // Left table is cheaper to build!
                        exec_order[idx_L] = r_table;
                        exec_order[idx_R] = l_table;

                        build_table = l_table;
                        build_expr = (l_table == left_dep) ? left : right;
                        probe_expr = (l_table == left_dep) ? right : left;
                    }
                }

                join_probe_exprs[build_table] = probe_expr;

                // Build the Map for the selected table
                ...
                while (true) {
                    ...
                    if (local_filters[build_table]) {
                        sql_node_t *res = sql_eval(ctx, local_filters[build_table]);
                        if (!res || res->is_null || !res->value.bool_value) continue;
                    }
                    ...
                    group_node_t lookup = {0};
                    lookup.key = key_val;
                    group_node_t *found = group_map_find(join_maps[build_table], &lookup);
                    if (!found) {
                        found = aml_pool_zalloc(ctx->pool, sizeof(group_node_t));
                        found->key = copy_evaluated_node(ctx, key_val);
                        group_map_insert(&join_maps[build_table], found);
                    }
                ...
                // Condition satisfied by map probe, delete from execution loop
                join_conds[r_table] = NULL;
            }
        }
        jp = jp->next;
    }
