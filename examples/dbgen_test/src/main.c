...
#define COL_INT(T_NAME, C_NAME) \
    static sql_node_t *col_##T_NAME##_##C_NAME(sql_ctx_t *ctx, sql_node_t *f) { \
        ...
        T_NAME##_t *r = (T_NAME##_t *)((void **)target_ctx->row)[f->column->table_index]; \
...
int main(int argc, char **argv) {
    const char *queries[] = {
        ...
        "LIMIT 5",
        // Test 22: Correlated EXISTS (Mildly Slow)
        "SELECT n.name "
        "FROM nation n "
        "WHERE EXISTS ("
        "    SELECT 1 FROM customer c "
        "    WHERE c.nationkey = n.nationkey AND c.acctbal > 9990"
        ")",
        // Test 23: Correlated EXISTS (Melt the CPU)
